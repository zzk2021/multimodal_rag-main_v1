...
DEFAULT_EMBEDDING_MODEL = config["embedding_model_path"]
class FLMREmbedding(MultiModalEmbedding):
    """
        FLMR embedding models for encoding text and image for Multi-Modal purpose.
    """
    ...
    def __init__(
        ...
        *,
        ...
        image_embedding_model: str = DEFAULT_EMBEDDING_MODEL,
        **kwargs: Any,
    ):
        ...
        try:
            ...
            image_processor_name = image_embedding_model
            self.query_tokenizer = FLMRQueryEncoderTokenizer.from_pretrained(checkpoint_path, subfolder="query_tokenizer")
            self.context_tokenizer = FLMRContextEncoderTokenizer.from_pretrained(
                checkpoint_path, subfolder="context_tokenizer"
            )

            self._model = FLMRModelForRetrieval.from_pretrained(
                checkpoint_path,
                query_tokenizer=self.query_tokenizer,
                context_tokenizer=self.context_tokenizer,
            )
            self._model.to(self._device)
            self._preprocess = AutoImageProcessor.from_pretrained(image_processor_name)
    ...
    def _get_text_embeddings(self, texts: List[str]) -> List[Embedding]:
        results = []
        inputs = self.context_tokenizer(texts)
